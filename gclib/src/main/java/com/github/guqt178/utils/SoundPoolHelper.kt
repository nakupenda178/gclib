...
class SoundPoolHelper @JvmOverloads constructor(
    ...
    private fun loadDefault() {
        ...
        if (uri == null)
            ...
            load(DEFAULT_NAME, uri2Path(context, uri) ?: "")
    }
    ...
    private fun play(ringtoneName: String, isLoop: Boolean) {
        if (ringtoneIds.containsKey(ringtoneName)) {
            soundPool?.play(ringtoneIds[ringtoneName] ?: -1, 1f, 1f, 1, if (isLoop) -1 else 0, 1f)
        }
    }
    ...
    companion object {
        ...
        ) = SoundPoolHelper(context, maxStream, streamType)

        @JvmOverloads
        fun play(context: Context) = get(context)
