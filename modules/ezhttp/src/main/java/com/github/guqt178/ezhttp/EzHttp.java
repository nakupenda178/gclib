...
package com.github.guqt178.ezhttp;
...
import com.github.guqt178.ezhttp.cache.converter.IDiskConverter;
import com.github.guqt178.ezhttp.cache.converter.SerializableDiskConverter;
import com.github.guqt178.ezhttp.cache.model.CacheMode;
import com.github.guqt178.ezhttp.cookie.CookieManger;
import com.github.guqt178.ezhttp.https.HttpsUtils;
import com.github.guqt178.ezhttp.interceptor.HttpLoggingInterceptor;
import com.github.guqt178.ezhttp.model.HttpHeaders;
import com.github.guqt178.ezhttp.model.HttpParams;
import com.github.guqt178.ezhttp.request.CustomRequest;
import com.github.guqt178.ezhttp.request.DeleteRequest;
import com.github.guqt178.ezhttp.request.DownloadRequest;
import com.github.guqt178.ezhttp.request.GetRequest;
import com.github.guqt178.ezhttp.request.PostRequest;
import com.github.guqt178.ezhttp.request.PutRequest;
 ...
 * 版本： v1.0<br>
 */
public final class EzHttp {
    private static Application sContext;
    public static final int DEFAULT_MILLISECONDS = 6 * 1000;          //默认的超时时间
    ...
    private volatile static EzHttp singleton = null;
    ...
    private EzHttp() {
        ...
        rxCacheBuilder = new RxCache.Builder().init(sContext).diskConverter(new SerializableDiskConverter());//目前只支持Serializable和Gson缓存其它可以自己扩展
    }

    public static EzHttp getInstance() {
        testInitialize();
        if (singleton == null) {
            synchronized (EzHttp.class) {
                if (singleton == null) {
                    singleton = new EzHttp();
                }
            }
        }
        return singleton;
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public EzHttp debug(String tag) {
        debug(tag, true);
        return this;
    }
     ...
     */
    public EzHttp debug(String tag, boolean isPrintException) {
        String tempTag = TextUtils.isEmpty(tag) ? "EzHttp_" :tag;
        if(isPrintException){
     ...
     * https的全局访问规则
     */
    public EzHttp setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public EzHttp setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public EzHttp setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public EzHttp setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public EzHttp setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public EzHttp setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public EzHttp setConnectTimeout(long connectTimeout) {
     ...
     * 超时重试次数
     */
    public EzHttp setRetryCount(int retryCount) {
     ...
     * 超时重试延迟时间
     */
    public EzHttp setRetryDelay(int retryDelay) {
     ...
     * 超时重试延迟叠加时间
     */
    public EzHttp setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 全局的缓存模式
     */
    public EzHttp setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存过期时间
     */
    public EzHttp setCacheTime(long cacheTime) {
     ...
     * 全局的缓存大小,默认50M
     */
    public EzHttp setCacheMaxSize(long maxSize) {
     ...
     */
    public EzHttp setCacheVersion(int cacheersion) {
        if (cacheersion < 0)
     ...
     */
    public EzHttp setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public EzHttp setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public EzHttp setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public EzHttp addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public EzHttp addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局拦截器
     */
    public EzHttp addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public EzHttp addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public EzHttp setOKProxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public EzHttp setOKConnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public EzHttp setOKClient(OkHttpClient client) {
     ...
     */
    public EzHttp addConverterFactory(Converter.Factory factory) {
     ...
     */
    public EzHttp addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public EzHttp setCallbackExecutor(Executor executor) {
     ...
     */
    public EzHttp setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public EzHttp setBaseUrl(String baseUrl) {
