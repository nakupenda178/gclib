...
import com.github.guqt178.ezhttp.EzHttp;
import com.github.guqt178.ezhttp.api.ApiService;
...
import com.github.guqt178.ezhttp.interceptor.BaseDynamicInterceptor;
import com.github.guqt178.ezhttp.interceptor.CacheInterceptor;
import com.github.guqt178.ezhttp.interceptor.CacheInterceptorOffline;
import com.github.guqt178.ezhttp.interceptor.HeadersInterceptor;
import com.github.guqt178.ezhttp.interceptor.NoCacheInterceptor;
...
import static com.github.guqt178.ezhttp.EzHttp.getRetrofitBuilder;
import static com.github.guqt178.ezhttp.EzHttp.getRxCache;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = EzHttp.getContext();
        EzHttp config = EzHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = EzHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = EzHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = EzHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) EzHttp.getCookieJar().addCookies(cookies);
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = EzHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = EzHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(EzHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, EzHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(EzHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(EzHttp.getContext(), cacheControlValue);
