...
fun <T, R> T.exeAsyncAction(
    task: AnkoAsyncContext<T>.() -> R,
    onTask: OnTask<T, R>? = null
) {
    val context = AnkoAsyncContext(WeakReference(this))

    ContextHelper.handler.post { onTask?.onStart(this) }
    BackgroundExecutor.submit {
        try {
            val result = context.task()
            ContextHelper.handler.post { onTask?.onResult(result) }
        } catch (thr: Throwable) {
            thr.printStackTrace()
            ContextHelper.handler.post { onTask?.onError(thr) }
        }
    }
}

open class  SimpleOnTask<in T,R> :OnTask<T, R>{
    override fun onStart(t: T) {}
    override fun onResult(r: R) {}
    override fun onError(th: Throwable) {}
}

interface OnTask<in T, R> {
    fun onStart(t: T)
    fun onResult(r: R)
